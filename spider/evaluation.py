...
def evaluate(gold, predict, db_dir, etype, kmaps):
    gold_encoding = detect_encoding(gold)
    pred_encoding = detect_encoding(predict)
    with open(gold, 'r', encoding=gold_encoding) as f:
        glist = [l.strip().split('|') for l in f.readlines() if len(l.strip()) > 0]
    with open(pred, 'r', encoding=pred_encoding) as f:
